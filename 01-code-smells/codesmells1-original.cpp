...
Explanation: Shotgun Surgery occurs when a single responsibility is scattered 
across multiple classes or methods. When you need to make a small change 
(e.g., adding a new feature or fixing a bug), you have to make modifications 
in many different places. This is a sign of poor cohesion and makes the system 
more prone to errors, as missing one of the changes can lead to bugs.
*/
...
/*
Smell: A method in one class is more interested in the data or functionality of
 another class than its own.
Explanation: Feature Envy occurs when a method accesses more data from another 
class than from its own class, indicating that it may be better placed in the 
class it "envies." This smell violates the principle of encapsulation because the
 method doesn't make effective use of its own class's data or functionality.
*/
class Order {
    double price;
    int itemCount;
